...
states <- st_read("../Data/GIS/west_coast_states.shp", crs = 4326)
...
# krill illustrations
tspin_logo <- readPNG("./Outputs/TSPIN.png", TRUE)
epac_logo <- readPNG("./Outputs/EPAC.png", TRUE)
```

Krill data
```{r}
load("../1-Krill_exploration/Outputs/krill_df.RData")
load("../5-Krill_bongo_SDM/Outputs/biomass_df.RData")
load("../5-Krill_bongo_SDM/Outputs/biomass_nhl_df.RData")
load("../3-Krill_SDM/Outputs/krill_day_df.RData")
```

Models
```{r}
# NASC total krill SDM
load("../3-Krill_SDM/Outputs/ENScurves_krill_dynlogdep.RData")
load("../3-Krill_SDM/Outputs/influ_gam_dynlogdep.RData")
load("../3-Krill_SDM/Outputs/influ_brt_dynlogdep.RData")
load("../3-Krill_SDM/Outputs/predyear_crossENSavg_dynlogdep.RData")

# biomass SDM
load("../5-Krill_bongo_SDM/Outputs/ENScurves_krillmass_topolog.RData")
load("../5-Krill_bongo_SDM/Outputs/influ_brt_topolog_mass.RData")
load("../5-Krill_bongo_SDM/Outputs/influ_gam_topolog_mass.RData")
load("../5-Krill_bongo_SDM/Outputs/predyear_crossENSavg_topolog_mass.RData")
load("../5-Krill_bongo_SDM/Outputs/all_biomass_eval_df.RData")

# final prediction
load("../5-Krill_bongo_SDM/Outputs/pred_nascXmass_ENSENStopolog.RData")
load("../5-Krill_bongo_SDM/Outputs/krill_final_ENSxENStopolog_trend.RData")
load("../5-Krill_bongo_SDM/Outputs/krill_nhl_pred_ENSENStopolog_trend.RData")
```
...
krill_day_df %>% 
  group_by(CruiseID) %>% 
  summarize(days = paste(unique(Date_M)))
...
ggsave(g, file = "inset_fig.png", bg = "transparent", units = "mm", width = 50, height = 40, dpi = 800)
```
...
north_america <- readPNG("./inset_fig.png", TRUE)
...
g2b1 <- ENScurves_krill_dynlogdep %>% 
  ...
  mutate(x = ifelse(predvar == "DEPTH", exp(x), x), # reverse log(DEPTH)
         x = ifelse(predvar == "EKE", 10^x - 0.01, x)) %>% # reverse log10(x + 0.01)
...
g4b <- ENScurves_krillmass_topolog %>% 
  ...
  #filter(predvar %in% bin_top2$var[1:3]) %>%
...
g4c <-ENScurves_krillmass_topolog %>% 
  ...
  mutate(id = paste(folds, group, sep = "_"),
         predvar = car::recode(predvar, "'DIS_CANYON' = 'CANYON'; 
           'logDEPTH' = 'DEPTH'", as.numeric = F),
         group = car::recode(group, "'EPAC' = 'Epac'; 'TSPIN' = 'Tspin'", as.numeric = F)) %>% 
  #filter(predvar %in% abu_top2$var) %>%
  mutate(predvar = factor(predvar, levels = c("DEPTH" , "SLOPE" , "CANYON"))) %>% 
  mutate(x = ifelse(predvar == "DEPTH", exp(x), x)) %>%  # reverse log(DEPTH)
  ggplot(aes(x, fit, group = id, col = group)) +
...
######### EPAC and TSPIN 60 % areas #######################
# this is not working because the 60 % never overlap for both species
g5c <- ggplot() +
    geom_tile(data = subset(d_epacprop, period == "Feb 2022" & fit >= 0.6), aes(x, y), fill = krillsp_pal[1], alpha = 0.8) +
    geom_tile(data = subset(d_tspinprop, period == "Feb 2022" & fit >= 0.5), aes(x, y), fill = krillsp_pal[2], alpha = 0.8) +
    geom_sf(data = iso_sf_utm[iso_sf_utm$g_2020_ %in% c(-200, -500, -1000, -1500), ], 
            col = "grey80", linewidth = 0.1) +
    geom_sf(data = coast_sf_utm, col = NA, fill = "grey50") +
    geom_sf(data = states, fill = NA, col = "grey30", size = 0.1) +
...
quantile(d_tspinprop$fit, 0.5)
quantile(d_epacprop$fit, 0.5)
...
g <- ggplot(d, aes(month, meansum, color = group, group = group)) +
    geom_point() +
...
biomass_allnhl_df <- readxl::read_excel("../Data/Fisher data/NHL_krill_biomass_2001-2022_sum_LH_w_LatLon.xlsx") %>%
...
g1 <- ggplot(d1, aes(log(-DEPTH + 1), mean_pred, color = group)) +
    ...
    geom_ribbon(aes(fill = group, ymin = mean_pred + se_pred, ymax = mean_pred - se_pred), 
                color = "transparent", alpha = 0.5) + #alternative is  ymin = min_pred, ymax = max_pred
    geom_path(alpha = 0.9) +
...
g <- g1 / (g2 | g3) &
  ...
  theme(legend.position = "bottom")
g
ggsave(g, file = "./Outputs/figure8.png", width = 140, height = 140, dpi = 500, units = "mm")
```
...
# option 2
### option where the dashed lines are the 2001-2022 average and ribbons / full line path are 2018-22
g3 <- ggplot(bydepth_18_22_nhl_df, aes(log(-DEPTH + 1), mean_obs, color = group)) +
    geom_ribbon(aes(fill = group, ymin = mean_obs + se_obs, ymax = mean_obs - se_obs), 
                color = "transparent", alpha = 0.5) +
    geom_path(alpha = 0.9) +
    geom_path(data = bydepth_all_nhl_df, 
              aes(log(-DEPTH + 1), mean_obs, group = group, 
                  color = group), lty="21", linewidth = 0.5) +
    scale_y_sqrt(name = expression(paste("Observed biomass (mg/", m^{3}, ")", sep = ""))) +
    scale_color_manual(name = "Species", values = krillsp_pal, guide = "none") +
    scale_fill_manual(name = "Species", values = krillsp_pal, guide = "none") +
    mon_theme +    
...
# correlation plot
png("./Outputs/supmat/correlations_krill_day_df.png", res = 600, units = "mm", width = 160, height = 160)
...
g <- ggplot() +
    ...
    guides(fill = guide_colorbar(title.position="top", title.hjust = 0.5))
g
ggsave(g, file = "./Outputs/supmat/NNC and NHLine station.png", width = 80, height = 140, dpi = 500, units = "mm")
```
...
g <- ggplot() +
    ...
    guides(fill = guide_colorbar(title.position="top", title.hjust = 0.5))
g
ggsave(g, file = "./Outputs/supmat/krill_day_df_percruise.png", width = 180, height = 240, dpi = 500, units = "mm")
```


Validation of predicted proportions of Epac and Tspin, using both the NH line and the NCC stations as observed data
```{r}
...
a1 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_TSPIN", y = "tspin_ENSdynlogdep", method = "ENS - dynamic_logdepth", sp = "Tspin")
b1 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_EPAC", y = "epac_ENSdynlogdep", method = "", sp = "Epac")

a2 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_TSPIN", y = "tspin_BRTdynlogdep", method = "BRT - dynamic_logdepth", sp = "Tspin")
b2 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_EPAC", y = "epac_BRTdynlogdep", method = "", sp = "Epac")

a3 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_TSPIN", y = "tspin_GAMdynlogdep", method = "GAM - dynamic_logdepth", sp = "Tspin")
b3 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_EPAC", y = "epac_GAMdynlogdep", method = "", sp = "Epac")

a4 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_TSPIN", y = "tspin_BRTtopo", method = "BRT - topographic", sp = "Tspin")
b4 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_EPAC", y = "epac_BRTtopo", method = "", sp = "Epac")

a5 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_TSPIN", y = "tspin_ENStopo", method = "ENS - topographic_logdepth", sp = "Tspin")
b5 <- Fun_evalprop(all_biomass_eval_df, 
                   x = "mass_EPAC", y = "epac_ENStopo", method = "", sp = "Epac")
...
g <- (((a1 + b1) / (a2 + b2) / (a3 + b3)) | ((a4 + b4) / (a5 + b5) / (a6 + b6))) &
  theme(text = element_text(size = 6))

ggsave(g, file = "./Outputs/supmat/obs_vs_predicted_proportions.png", width = 240, height = 240, dpi = 500, units = "mm")
```
...
ggsave(g5, file = "./Outputs/supmat/predicted_proportions_entiredomain.png", width = 110, height = 110, dpi = 300, units = "mm")
g5
```
...
g1 <- ggplot(d1, aes(month, mean_pred, color = group, group = group)) +
    geom_ribbon(aes(fill = group, ymin = mean_pred - sd_pred, ymax = mean_pred + sd_pred), 
                color = "transparent", alpha = 0.5) + #alternative is mean_pred + sd_pred
    geom_path(alpha = 0.9) +
    scale_y_sqrt("Predicted Species-scaled NASC") +
    scale_color_manual(name = "Species", values = krillsp_pal, 
             labels = c(expression(italic("E. pacifica")), expression(italic("T. spinifera")))) +
...
g <- (g1 / g2) &
  plot_annotation(tag_levels = "a")
ggsave(g, file = "./Outputs/supmat/NHLine_validation_time.png", width = 140, height = 140, dpi = 500, units = "mm")
```
...
Other options tested for the plot S7
```{r, eval = F}
#### option 1 
g2 <- ggplot(bymonth_18_22_nhl_df, aes(month, mean_obs, color = group, fill = group)) +
    geom_ribbon(data = bymonth_all_nhl_df,
                aes(as.numeric(month), ymin = mean_obs - se_obs, 
                    ymax = mean_obs + se_obs),
                color = "transparent", alpha = 0.5) +
    geom_path(data = bymonth_all_nhl_df,
              aes(month, mean_obs, group = group, 
                  color = group), lty = "21", linewidth = 0.5) +
    geom_col(position = "dodge", alpha = 0.6) +
    geom_errorbar(aes(ymin = mean_obs, 
                         ymax = mean_obs + se_obs),
                      position = position_dodge2(width = 0.2, padding = 0.5)) +
    scale_y_sqrt(name = expression(paste("Observed biomass (mg/", m^{3}, ")", sep = "")))+
    scale_color_manual(name = "Species", values = krillsp_pal, guide = "none") +
...
g <- (g1 / g2) &
  plot_annotation(tag_levels = "a")
ggsave(g, file = "./Outputs/supmat/NHLine_validation_time_option1.png", width = 140, height = 120, dpi = 500, units = "mm")

### option 2
g4 <- ggplot(climato_all_nhl_df, aes(month, mean_obs, color = group, fill = group)) +
    geom_col(position = "dodge", alpha = 0.6) +
    geom_errorbar(aes(ymin = mean_obs, 
                         ymax = mean_obs + sd_obs),
                      position = position_dodge2(width = 0.2, padding = 0.5)) +
    scale_y_sqrt(name = expression(paste("Observed biomass 2001-2022 (mg/", m^{3}, ")", sep = "")))+
    scale_color_manual(name = "Species", values = krillsp_pal, guide = "none") +
    scale_fill_manual(name = "Species", values = krillsp_pal, guide = "none") +
    mon_theme
...
g <- (g1 / g2 / g4) &
  plot_annotation(tag_levels = "a")
ggsave(g, file = "./Outputs/supmat/NHLine_validation_time_option2.png", width = 120, height = 180, dpi = 500, units = "mm")
```
